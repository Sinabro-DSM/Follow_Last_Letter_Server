...
hardList = ['강아지', '망아지', '송아지', '소화기', '연예인', '가이드', '대통령']
...
def verify(word):
    ...
    :return:
    """
    tmp = dueum(wordList[-1])

    print(tmp[-1])

    if tmp[-1] != word[0] or word in wordList or word not in words:
        return False
    else:
        return True


def find_easy(letter):
    """
    쉬운 난이도 단어 받아오기
    :return:
    """
...
def easy():
    ...
    if check:
        ...
        print(wordList)
        return jsonify(check, resWord, int(et - st)), 200
    else:
        print(wordList)
